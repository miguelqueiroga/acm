 ...
 */
public class MyNumberValidator extends Validator
{
     ...
     */
    public static function validateNumber(validator:MyNumberValidator,
										  value:Object,
										  baseField:String):Array
    {
        ...
        if (!isNaN(minValue) || !isNaN(maxValue))
        {
			...
			var numero:String = input;
			numero = numero.replace(myPattern2, "?").replace(myPattern, ",").replace(myPattern3, ".");
			//Alert.show(numero);
			//Alert.show(input);
			//Alert.show(minValue + "");
			//Alert.show(maxValue + "");
            ...
            var x:Number = parseFloat(numero);
            //Alert.show(x + "");

            if (isNegative)
                x = -x;
			//Alert.show(!isNaN(minValue) + "");
			//Alert.show(!isNaN(maxValue) + "");
			//Alert.show((x < minValue) + "");
			//Alert.show((x > maxValue) + "");
	 ...
	 *
	 *  @default "O valor digitado é maior que o máximo permitido."
     */
	 ...
	 *
	 *  @default "Este campo precisa ser informado."
     */
    ...
    override protected function doValidation(value:Object):Array
    {
		...
		if (results.length > 0 || ((val.length == 0) && !required))
		    ...
		    return MyNumberValidator.validateNumber(this, value, null);
    }
}
