...
public class ServletReportDemonstrativoFinanceiro extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		String nomeRelatorio = "WEB-INF/relatorios/Relatorio_Financeiro.jasper";
		...
		try{
			...
			if (socioId != null && !socioId.equals("") && !socioId.equals("NaN")) {
				...
				parameters.put("examesContabilizados", getQuantidadeExamesPorSocio(socioId, dataInicial, dataFinal, resumos) + getQuantidadeExamesSemValorPorSocio(socioId, dataInicial, dataFinal, resumos));
				receitasAvulsas = getReceitasAvulsasPorSocia(socioId, dataInicial, dataFinal);
				retiradas = getRetiradasPorPeriodoSocio(socioId, dataInicial, dataFinal);
				aplicacoes = getAplicacoesPorPeriodoSocio(socioId, dataInicial, dataFinal);
			}	
			else {
				nomeRelatorio = "WEB-INF/relatorios/Relatorio_Financeiro_Geral.jasper";	
				parameters.put("totalRecebido", getValorRecebidoPorPeriodoGeral(dataInicial, dataFinal));
				resumos = getResumoMensalGeral(dataInicial, dataFinal);				
				parameters.put("examesContabilizados", getQuantidadeExamesGeral(dataInicial, dataFinal, resumos) + getQuantidadeExamesGeralSemValor(dataInicial, dataFinal, resumos));
				receitasAvulsas = getReceitasAvulsasGeral(dataInicial, dataFinal);
				retiradas = getRetiradasPorPeriodoGeral(dataInicial, dataFinal);
				aplicacoes = getAplicacoesPorPeriodoGeral(dataInicial, dataFinal);
			}	
			...
			parameters.put("totalRetiradas", ReportUtils.getTotalRetiradas(retiradas));
			parameters.put("totalAplicacoes", ReportUtils.getTotalAplicacoes(aplicacoes));
	...
	protected Integer getQuantidadeExamesGeral(String dataInicial, String dataFinal, List<ResumoMensal> resumos) {
		return ReportUtils.getQuantidadeExamesComValorPorPeriodoGeral(dataInicial, dataFinal, getReceitasPorResumos(resumos));
	}

	protected Integer getQuantidadeExamesGeralSemValor(String dataInicial, String dataFinal, List<ResumoMensal> resumos) {
		return ReportUtils.getQuantidadeExamesSemValorPorPeriodoGeral(dataInicial, dataFinal, getReceitasPorResumos(resumos));
	}

	protected Double getQuantidadeExamesPorSocio(String socioId, String dataInicial,
			String dataFinal, List<ResumoMensal> resumos) {
		return ReportUtils.getQuantidadeExamesConvenioComValorPorPeriodoSocio(socioId, dataInicial, dataFinal, getReceitasPorResumos(resumos));
	}

	protected Double getQuantidadeExamesSemValorPorSocio(String socioId, String dataInicial,
			String dataFinal, List<ResumoMensal> resumos) {
		return ReportUtils.getQuantidadeExamesConvenioSemValorPorPeriodoSocio(socioId, dataInicial, dataFinal, getReceitasPorResumos(resumos));
	}

	private List<Receita> getReceitasPorResumos(List<ResumoMensal> resumos) {
		List<Receita> receitas = new ArrayList<Receita>();
		for (ResumoMensal resumo: resumos) {
			receitas.addAll(resumo.getReceitas());
		}
		return receitas;
	}
	...
	protected List<ResumoMensal> getResumoMensalPorSocia(String socioId,
		...
		for(DepartamentoSocio ds : departamentosSocios){
			...
			if(!(resumo.getValorReceita().equals("0,00") && resumo.getValorReceitaSemValorAVista().equals("0,00") && resumo.getValorDespesa().equals("0,00"))){
				resumos.add(resumo);
			}
		}
		return resumos;
	...
	protected List<ResumoMensal> getResumoMensalGeral(String dataInicial, String dataFinal) {
		...
		for(Departamento ds : departamentosSocios){
			...
			resumo.setValorReceitas(receitas);
			resumo.setValorDespesas(despesas);
			if(!(resumo.getValorReceitaDireta().equals("0,00") && resumo.getValorReceitaDiretaSemValorAVista().equals("0,00") && resumo.getValorDespesaDireta().equals("0,00"))){
				resumos.add(resumo);
			}
		}
		return resumos;
	...
	protected double getValorRecebidoPorPeriodoGeral(String dataInicial, String dataFinal) {
		double total = 0.0;
		List<Receita> receitas = new ArrayList<Receita>();
		List<ContaPagamento> contasSocios = null;
		String consulta = "";
		try {
			consulta = "From ACM.model.entity.ContaPagamento as conta where conta.cpSocio.socId is not null"; 
			System.out.println(factoryService.getListPorConsulta(consulta));
			contasSocios = factoryService.getListPorConsulta(consulta);
			for(ContaPagamento ct : contasSocios){
				receitas.addAll(getValorRecebidoContaPorIntervalo(ct, dataInicial, dataFinal));
	...
	protected List<Retirada> getRetiradasPorPeriodoSocio(String socioId, String dataInicial, String dataFinal) {
		List<Retirada> retiradas = new ArrayList<Retirada>();
		String consulta = "";
		try {
			//System.out.println(consulta);
	...
	protected List<Retirada> getRetiradasPorPeriodoGeral(String dataInicial, String dataFinal) {
		List<Retirada> retiradas = new ArrayList<Retirada>();
		String consulta = "";
		try {
			//System.out.println(consulta);
			consulta = "From ACM.model.entity.Retirada as r where r.vencimento.venNome BETWEEN '" + dataInicial + "' AND '" + dataFinal + "'"; 
			retiradas = factoryService.getListPorConsulta(consulta);			
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected List<Aplicacao> getAplicacoesPorPeriodoSocio(String socioId, String dataInicial, String dataFinal) {
		List<Aplicacao> aplicacoes = new ArrayList<Aplicacao>();
		String consulta = "";
		try {
			//System.out.println(consulta);
	...
	protected List<Aplicacao> getAplicacoesPorPeriodoGeral(String dataInicial, String dataFinal) {
		List<Aplicacao> aplicacoes = new ArrayList<Aplicacao>();
		String consulta = "";
		try {
			//System.out.println(consulta);
			consulta = "From ACM.model.entity.Aplicacao as r where r.vencimento.venNome BETWEEN '" + dataInicial + "' AND '" + dataFinal + "'"; 
			aplicacoes = factoryService.getListPorConsulta(consulta);			
		} catch (Exception e) {
			e.printStackTrace();
		}
