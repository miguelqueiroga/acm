...
public class ServletReportDinheiroAReceberSocio extends HttpServlet {
	...
	protected void service(HttpServletRequest request,
		...
		try{
			...
			financas.addAll(getRetiradasPorPeriodoSocio(socioId, dataInicial, dataFinal));
			financas.addAll(getAplicacaosPorPeriodoSocio(socioId, dataInicial, dataFinal));
			financas.addAll(getValorRecebidoPorPeriodoSocio(socioId, dataInicial, dataFinal));
			...
			reportFile = new File(sc.getRealPath("WEB-INF/relatorios/Relatorio_Geral_Final_Socio.jasper"));
	...
	protected List<FinancasSocioImpressao> organizaValoresRecebidosPorVencimento(
			List<Receita> receitas) {
		List<FinancasSocioImpressao> financas = new ArrayList<FinancasSocioImpressao>();
		HashMap<String, ArrayList<Receita>> receitasPorVencimento = new HashMap<String, ArrayList<Receita>>();
		for(Receita rec : receitas){ 
		...
		for(String vencimento : receitasPorVencimento.keySet()){
			FinancasSocioImpressao fsi = new FinancasSocioImpressao();
			fsi.setVencimento(vencimento);
			fsi.setNome("(-) Valores Recebidos em Conta PF");
			double valor = 0.0;
			for(Receita r : receitasPorVencimento.get(vencimento)){
				valor += Tools.stringToDouble(r.getValorRateado());
			}
	...
	protected List<FinancasSocioImpressao> getValorRecebidoPorPeriodoSocio(String socioId, String dataInicial, String dataFinal) {
		...
		return organizaValoresRecebidosPorVencimento(receitas);
	}
